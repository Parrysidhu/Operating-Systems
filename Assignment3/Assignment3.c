...
int __init init_module()
{
	...
	for(vma=pid_mm_struct->mmap; vma; vma=vma->vm_next) {
		for(vaddr=vma->vm_start; vaddr < vma->vm_end; vaddr++) {
			...
			if(pgd_none(*pgd)) {
				printk(KERN_ALERT "not found- pgd\n");
				return -1;
			} else {
				printk(KERN_ALERT "pgd value = 0x%lx\n", pgd_val(*pgd));
			}

			p4d = p4d_offset(pgd, vaddr);
			if(p4d_none(*p4d)) {
				printk(KERN_ALERT "not found- p4d\n");
				return -1;
			} else {
				printk(KERN_ALERT "p4d value = 0x%lx\n", p4d_val(*p4d));
			}

			pud = pud_offset(p4d, vaddr);
			if(pud_none(*pud)) {
				printk(KERN_ALERT "not found- pud\n");
				return -1;
			} else {
				printk(KERN_ALERT "pud value = 0x%lx\n", pud_val(*pud));
			}

			pmd = pmd_offset(pud, vaddr);
			if(pmd_none(*pmd)) {
				printk(KERN_ALERT "not found- pmd\n");
				return -1;
			} else {
				printk(KERN_ALERT "pmd value= 0x%lx\n", pmd_val(*pmd));
			}

			pte = pte_offset_map(pmd, vaddr);
			if(pte_present(*pte)) {
				pfn = pte_pfn(*pte);
				printk(KERN_ALERT "Frame no: %lu\n", pfn);
				return 0;
			}
		}
	}
